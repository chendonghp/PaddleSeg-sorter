...
def export_load_infer(args, input_data, model=None):
    """
    ...
    np.testing.assert_allclose(onnx_out, paddle_out, rtol=0, atol=ATOL)
    ...
    np.testing.assert_allclose(trt_out, paddle_out, rtol=0, atol=ATOL)
    print("The paddle and trt models have the same outputs.\n")

    return latency


def load_infer(args, input_data):
    ...
    np.testing.assert_allclose(trt_out, onnx_out, rtol=0, atol=ATOL)
    print("The onnx and trt models have the same outputs.\n")

def predict(arg, input="/home/cd/PaddleSeg/data/sorter/images/frame_001291.PNG"):
    im = cv2.imread(input)
    im = np.transpose(im, [2, 0, 1])
    im = np.expand_dims(im, axis=0)
    # https://aitechtogether.com/ai-question/35227.html#group=0&photo=0
    im = np.ascontiguousarray(im, dtype='float32')
    load_infer(args, im)
